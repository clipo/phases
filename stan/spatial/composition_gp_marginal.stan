// Decorated-ceramic composition as a latent spatial field (marginalised nugget).
//
// MARGINALISED-NUGGET VARIANT, written 2026-08-31 to fix a geometry defect.
//
// The original carries TWO additive latent matrices, u (iid) and f (GP), each
// N x C. That is 2 x 29 x 9 = 522 latent parameters against 29 x 9 = 261
// effective observations, and the two are additive at every site, so variance
// moves freely between them except where spatial correlation distinguishes
// them. As rho shrinks f becomes iid and is indistinguishable from u; as rho
// grows f becomes constant and is absorbed by alpha. That is a flat direction
// by construction, and it is what the sampler walked: 100 percent treedepth
// saturation with ZERO divergences, on simulated AND real data, under both the
// uniform and the resolvable-range prior.
//
// The fix is the standard GP-with-nugget form: one latent field drawn from
// Cholesky(K(rho, sigma) + tau^2 I). The nugget enters the covariance rather
// than as a second additive field, halving the latent dimension and removing
// the redundancy. tau stays identified through the diagonal inflation, and
// spatial_share keeps its meaning.
//
// Kept as a SEPARATE file rather than a switch so the two can be compared by
// measurement, following ../mataa's balding_nichols_logit.stan precedent.
//
...
transformed parameters {
  matrix[N, C] f;
  {
    matrix[N, N] Sigma = k_matern(D, sigma, rho, N, jitter, kernel);
    if (use_tau) {
      for (i in 1:N) Sigma[i, i] += square(tau[1]);
    }
    f = cholesky_decompose(Sigma) * z_f;
  }
}
model {
  ...
  if (likelihood) {
    for (i in 1:N) {
      vector[K] eta;
      eta[1] = 0;
      for (k in 2:K) eta[k] = alpha[k - 1] + f[i, k - 1];
