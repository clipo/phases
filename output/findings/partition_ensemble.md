...
Produced by `analyses/60_partition_ensemble.py` (full). F15 measurement M2. Basin, 29 assemblages, k = 3, Beta(1,10) prior, 150 random contiguous (Voronoi) partitions.
...
| | cultural F_ST |
|---|---|
| **the k-means partition we use** | **0.0179** [0.0156, 0.0201] |
| ensemble median | 0.0178 |
| ensemble 5th to 95th percentile | 0.0103 to 0.0265 |
| ensemble full range | 0.0043 to 0.0331 |
| **our partition's position in the ensemble** | **51th percentile** |

## Reading

Our partition sits at the 51th percentile, which is unremarkable. At this grain the specific partition k-means found is **one draw from a family of equally defensible partitions**, and the F_ST defined on it is not a property the data single out.
