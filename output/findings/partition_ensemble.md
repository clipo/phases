...
Produced by `analyses/60_partition_ensemble.py` (full). F15 measurement M2. Basin, 28 assemblages, k = 3, Beta(1,10) prior, 150 random contiguous (Voronoi) partitions.
...
| | cultural F_ST |
|---|---|
| **the k-means partition we use** | **0.0154** [0.0134, 0.0180] |
| ensemble median | 0.0126 |
| ensemble 5th to 95th percentile | 0.0064 to 0.0227 |
| ensemble full range | 0.0032 to 0.0265 |
| **our partition's position in the ensemble** | **73th percentile** |

## Reading

Our partition sits at the 73th percentile, which is unremarkable. At this grain the specific partition k-means found is **one draw from a family of equally defensible partitions**, and the F_ST defined on it is not a property the data single out.
