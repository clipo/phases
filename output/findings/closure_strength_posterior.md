...
Observed statistic: the rarefied F_ST trajectory rank correlation, T = **+0.0225**.

## Posterior over injected closure strength s

| time-averaging | median | 95% credible interval | P(s >= 0.3) | P(s >= 0.5) |
|---|---|---|---|---|
| none (w = 1) | **0.227** | [0.000, 0.481] | 0.371 | 0.025 |
| record's (w = 3) | **0.308** | [0.000, 0.646] | 0.550 | 0.201 |

## Rule 20(c): recovery of a closure the data do not contain

Data simulated at a true s = 0.7, well above anything the basin shows, returns a posterior median of **0.578** with interval [0.171, 0.907], which covers the truth.
...
The old apparatus said closure of strength 0.5 or more would be detected with power above 0.8, and read the empirical value as a nominal strength near 0.35. The posterior says the same thing without a null: strengths at or above 0.5 carry posterior mass 0.025 without time-averaging and 0.201 with it, so moderate and strong closure are excluded by the data rather than by a threshold, while weak closure remains credible. That is the bounded non-detection the paper argues for, stated as a parameter.
